...
EOF
    ...
    echo ""
    echo -e "${BOLD}${WHITE}[scanProxy]${OFF}"
read -r -d '' ops <<EOF
...
print_listAnsEntries_help() {
    ...
    echo -e ""
    echo -e "${BOLD}${WHITE}Parameters${OFF}"
    echo -e "  * ${GREEN}page_size${OFF} ${BLUE}[integer]${OFF} ${RED}(required)${OFF} ${CYAN}(default: null)${OFF} - ${YELLOW} Specify as: page_size=value${OFF}" \
        | paste -sd' ' | fold -sw 80 | sed '2,$s/^/    /'
    echo -e "  * ${GREEN}name_prefix${OFF} ${BLUE}[string]${OFF} ${CYAN}(default: null)${OFF} - ${YELLOW} Specify as: name_prefix=value${OFF}" \
        | paste -sd' ' | fold -sw 80 | sed '2,$s/^/    /'
    echo ""
    echo -e "${BOLD}${WHITE}Responses${OFF}"
    code=200
...
print_lookupTransferCommandStatus_help() {
    ...
    echo -e ""
    echo -e "Retrieve the status of all transfer commands of the given sender for the specified nonce." | paste -sd' ' | fold -sw 80
    echo -e ""
    echo -e "${BOLD}${WHITE}Parameters${OFF}"
...
call_getAmuletRules() {
    ...
    if ! path=$(build_request_path "/api/validator/v0/scan-proxy/amulet-rules" path_parameter_names query_parameter_names); then
        ERROR_MSG=$path
        exit 1
    fi
    local method="GET"
...
call_getAnsRules() {
    ...
    if ! path=$(build_request_path "/api/validator/v0/scan-proxy/ans-rules" path_parameter_names query_parameter_names); then
        ERROR_MSG=$path
        exit 1
    fi
    local method="POST"
...
call_getDsoPartyId() {
    ...
    if ! path=$(build_request_path "/api/validator/v0/scan-proxy/dso-party-id" path_parameter_names query_parameter_names); then
        ERROR_MSG=$path
        exit 1
    fi
    local method="GET"
...
call_getOpenAndIssuingMiningRounds() {
    ...
    if ! path=$(build_request_path "/api/validator/v0/scan-proxy/open-and-issuing-mining-rounds" path_parameter_names query_parameter_names); then
        ERROR_MSG=$path
        exit 1
    fi
    local method="GET"
...
call_listAnsEntries() {
    ...
    if ! path=$(build_request_path "/api/validator/v0/scan-proxy/ans-entries" path_parameter_names query_parameter_names); then
        ERROR_MSG=$path
        exit 1
    fi
    local method="GET"
...
call_lookupAnsEntryByName() {
    ...
    if ! path=$(build_request_path "/api/validator/v0/scan-proxy/ans-entries/by-name/{name}" path_parameter_names query_parameter_names); then
        ERROR_MSG=$path
        exit 1
    fi
    local method="GET"
...
call_lookupAnsEntryByParty() {
    ...
    if ! path=$(build_request_path "/api/validator/v0/scan-proxy/ans-entries/by-party/{party}" path_parameter_names query_parameter_names); then
        ERROR_MSG=$path
        exit 1
    fi
    local method="GET"
...
call_lookupFeaturedAppRight() {
    ...
    if ! path=$(build_request_path "/api/validator/v0/scan-proxy/featured-apps/{provider_party_id}" path_parameter_names query_parameter_names); then
        ERROR_MSG=$path
        exit 1
    fi
    local method="GET"
...
call_lookupTransferCommandCounterByParty() {
    ...
    if ! path=$(build_request_path "/api/validator/v0/scan-proxy/transfer-command-counter/{party}" path_parameter_names query_parameter_names); then
        ERROR_MSG=$path
        exit 1
    fi
    local method="GET"
...
call_lookupTransferCommandStatus() {
    ...
    if ! path=$(build_request_path "/api/validator/v0/scan-proxy/transfer-command/status" path_parameter_names query_parameter_names); then
        ERROR_MSG=$path
        exit 1
    fi
    local method="GET"
...
call_lookupTransferPreapprovalByParty() {
    ...
    if ! path=$(build_request_path "/api/validator/v0/scan-proxy/transfer-preapprovals/by-party/{party}" path_parameter_names query_parameter_names); then
        ERROR_MSG=$path
        exit 1
    fi
    local method="GET"
