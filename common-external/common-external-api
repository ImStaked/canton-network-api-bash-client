...
#
# This is a Bash client for Common schemas for Splice API definitions.
#
# LICENSE:
# 
#
...
${BOLD}${WHITE}Common schemas for Splice API definitions command line client (API version 0.0.1)${OFF}
...
print_about() {
    echo ""
    echo -e "${BOLD}${WHITE}Common schemas for Splice API definitions command line client (API version 0.0.1)${OFF}"
    echo ""
    echo -e "License: "
    echo -e "Contact: "
    echo ""
...
print_version() {
    echo ""
    echo -e "${BOLD}Common schemas for Splice API definitions command line client (API version 0.0.1)${OFF}"
    echo ""
}
...
call_isLive() {
    ...
    if ! path=$(build_request_path "/livez" path_parameter_names query_parameter_names); then
        ERROR_MSG=$path
        exit 1
    fi
    local method="GET"
...
call_isReady() {
    ...
    if ! path=$(build_request_path "/readyz" path_parameter_names query_parameter_names); then
        ERROR_MSG=$path
        exit 1
    fi
    local method="GET"
